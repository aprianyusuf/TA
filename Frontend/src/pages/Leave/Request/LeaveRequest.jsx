...
import { CALENDAR_VIEW } from "@/configs/constant";
...
const LeaveRequest = () => {
	...
	const userPosition = useBoundStore(
		(state) => state.user?.position?.name ?? "",
	);
			...
			columnHelper.accessor("status", {
				...
				cell: ({ getValue }) => {
					...
					return (
						<TableCell>
							<span
								className={`rounded-full px-3 py-1 text-sm font-semibold text-white ${bgColor}`}
							>
								{text}
							</span>
						</TableCell>
			...
			<Tabs defaultValue="table" className="flex h-full w-full flex-col">
				<TabsContent value="table">
					<div className="flex items-center justify-between">
						<h2 className="ml-5 text-xl font-bold">Leave Request</h2>
